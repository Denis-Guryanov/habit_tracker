...
class HabitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get('related_habit') and data.get('reward'):
            raise serializers.ValidationError('Можно указать только одно: либо связанную привычку, либо вознаграждение.')
        ...
        if data.get('related_habit') and not data['related_habit'].is_pleasant:
            raise serializers.ValidationError('В связанные привычки можно выбирать только приятные привычки.')
